...
export function RoleSwitcher() {
  ...
  const [activeRole, setActiveRole] = useState<UserRole>(user?.roles[0] || 'dosen');
      ...
      <Dialog open={showDialog} onOpenChange={setShowDialog}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Anda memiliki {user.roles.length} role. Pilih tampilan yang ingin Anda gunakan.
            </DialogDescription>
          </DialogHeader>
            ...
            {user.roles.map((role) => (
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <Badge className={roleBadgeColors[role]}>{roleLabels[role]}</Badge>
                  {activeRole === role && (
          ...
          <div className="flex items-center gap-2 p-3 bg-blue-50 border border-blue-200 rounded-lg text-sm">
            ...
            <p className="text-blue-900">
              Saat ini tampilan menggabungkan semua role Anda. Fitur switch role akan segera
              tersedia.
            </p>
          </div>
        </DialogContent>
