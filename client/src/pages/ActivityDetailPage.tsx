...
import {
  ArrowLeft, Calendar, DollarSign, Users, FileText,
  CheckCircle, AlertCircle, Eye, Highlighter, Edit, Trash2,
  Share2, History as HistoryIcon
} from 'lucide-react';
...
export function ActivityDetailPage() {
  ...
  const isReadOnlyView = user?.role === 'kaprodi' || user?.role === 'kajur' ||
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span>{format(new Date(activity.tanggalMulai), 'dd MMMM yyyy')}</span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-2">
                      <Calendar className="w-4 h-4 text-muted-foreground" />
                      <span>{format(new Date(activity.tanggalSelesai), 'dd MMMM yyyy')}</span>
                    </div>
                  </div>
                  ...
                  {activity.sumberDana && (
                    <div className="space-y-2">
                      <label className="text-sm font-medium text-muted-foreground">Sumber Dana</label>
                  ...
                  {activity.biaya > 0 && (
                    <div className="space-y-2">
                      <label className="text-sm font-medium text-muted-foreground">Biaya Kegiatan</label>
                      <div className="flex items-center gap-2">
                        <DollarSign className="w-4 h-4 text-muted-foreground" />
                        <span className="font-medium">{formatRupiah(activity.biaya)}</span>
                      </div>
                    </div>
                  )}
                </div>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground font-mono">NIDN: {dosen.nidn}</p>
                        </div>
                      </div>
                      ...
                      <div className="space-y-2 pl-12">
                        ...
                        {dosen.dokumen.map((doc) => (
                          ...
                          >
                              ...
                              <div>
                                ...
                                <div className="flex gap-2 mt-1">
                                  ...
                                  <span className="text-xs text-muted-foreground">
                                    Upload: {format(new Date(doc.tanggalUpload), 'dd MMM yyyy')}
                                  </span>
                                </div>
                              </div>
                            </div>
                            <div className="flex items-center gap-2">
                              {doc.hasHighlight && (
                                <Highlighter className="w-4 h-4 text-yellow-500" title="Ada highlight" />
                              )}
                              <Button
                                variant="ghost"
                                size="sm"
            ...
            <Card className="bg-muted/30">
              <CardContent className="pt-6">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
                  <div>
                    <p className="text-sm text-muted-foreground">Total Dosen Terlibat</p>
                    <p className="text-2xl font-bold">{activity.dosenTerlibat.length}</p>
                  </div>
                  <div>
                    <p className="text-sm text-muted-foreground">Total Dokumen Bukti</p>
                    <p className="text-2xl font-bold">
                      {activity.dosenTerlibat.reduce((sum, d) => sum + d.dokumen.length, 0)}
                    </p>
                  </div>
                  <div>
                    <p className="text-sm text-muted-foreground">Dosen Belum Upload</p>
                    <p className="text-2xl font-bold text-red-600">
                      {activity.dosenTerlibat.filter(d => d.dokumen.length === 0).length}
                    </p>
                  </div>
                </div>
              </CardContent>
